...
\Cref{fig:mdp-example} shows a graphical representation of an MDP. The circles represent the states of the environment and the arrows represent the transition probability of going from $s_t$ to $s_{t+1}$. Each state is associated with a reward, depicted in red. \\
...
The action-value function allows us to introduce another quantity of particular interest for the algorithms described in \Cref{sec:solve-mdp}, called the advantage function. The advantage of action $a$, state $s$, over policy $\pi$ encodes how much we can gain by performing action $a$ in state $s$ instead of following the policy $\pi$, and is defined as follows:
