...
\tikzstyle{fancytitle} =[fill=black!10!blue, text=black]
...
\def\heightfrommc{2.5}

\draw[scale=1,domain=0.5:4.5,smooth,variable=\x] plot ({\x},{\heightfrommc+1/(sqrt(2*3.14) * (\sigmam)) * e^(-1/2 * ((\x - \mum)/(\sigmam))^2)});
\draw[->] (2,\heightfrommc) -- (2,{\heightfrommc + 1.5}) node[left]{$\pi(a|s)$};
\draw[->] (0.5,\heightfrommc) -- (4.8,\heightfrommc) node[above]{$F$};
...
\def\columnwidth{7}
\draw[] (\columnwidth,\heightfrommc + 1) node{$\pi(a|s) = \frac{1}{\sqrt{2\pi}\sigma}\exp\left( -\frac{1}{2}\left(\frac{a-\boldsymbol{v}\cdot{s}}{\sigma}\right)^2 \right)$};

\node[mybox](box) at (\columnwidth, 0){asdf};
...
\draw[thick, red, -angle 90] (2.8,0.1) -- (3.1,0.46) node[above]{$F$};
